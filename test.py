...
NoOfSamples = 5000  # no. of observations or samples
...
def Alamouti_2byX():
    for k in range(0, itr):
        EbNodB = EbNodB_range[k] # 信噪比dB
        EbNo = 10.0 ** (EbNodB / 10.0) # 信噪比非dB
        noise_std = 1 / sqrt(2 * EbNo) # 噪声标准差   噪声方差 = 1/(2*SNR)
        ...
        for m in range(0, NoOfSamples):
            n = []
            tx_symbol1 = (2 * random.randint(0, 1) - 1)  # generate first BPSK symbols   -1/1
            tx_symbol2 = (2 * random.randint(0, 1) - 1)  # generate second BPSK symbols  -1/1
            ...
            for j in range(4):  # since, I am considering a 2by2 Alamouti Scheme, ie. 4 distinct Channel Coefficients
                ...
                nxx = np.random.normal(loc=noise_mean, scale=noise_std, size=None) - np.random.normal(loc=noise_mean,
                                                                                                      scale=noise_std,
                                                                                                      size=None) * 1j  ##specifically, noise with a pre-defined mean & standard deviation
                n.append(nxx)

            H = np.array([[h[0], h[1]], [h[2], h[3]], [-h[1].conj(), h[0].conj()], [-h[3].conj(), h[
                2].conj()]])  # i kinda manually concatenated the coefficient to assumed the form for an Alamouti Coding
            H_uncoded = np.array([[h[0], h[1]], [h[2], h[
                3]]])  # i kinda manually concatenated the coefficient to assumed the form for an Alamouti Coding

            H2by1 = np.array([[h[0], h[1]], [h[1].conj(), -h[
                0].conj()]])  # forms the coded channel matrix from the previosuly obtained matrix
            ...
            rx_symbol = np.add((np.dot(H, (np.dot((1 / sqrt(2)), symbol)))),
                               Noise)  # recieved_symbol - 2by2 Alamouti, y == rx_symbol

            rx_symbol_uncoded = np.add((np.dot(H_uncoded, (np.dot((1 / sqrt(2)), symbol)))),
                                       Noise_uncoded)  # recieved_symbol - 2by2 Alamouti, y == rx_symbol

            rx_symbol2by1 = np.add((np.dot(H2by1, (np.dot((1 / sqrt(2)), symbol)))),
                                   Noise2by1)  # recieved_symbol for 2by1 Alamouti
            ...
            Symbol_est2by1 = np.dot(np.linalg.pinv(H2by1), rx_symbol2by1)  # ZF检测
            ...
            no_errors_uncoded += 1 * (tx_symbol1 != det_symbol1_uncoded) + 1 * (
                        tx_symbol2 != det_symbol2_uncoded)  # 2by2
...
def Alamouti_4byX():
    for k in range(0, itr):
        ...
        for m in range(0, NoOfSamples):
            ...
            H = np.array([[h[0], h[1], h[2], h[3]], [h[4], h[5], h[6], h[7]], [h[8], h[9], h[10], h[11]],
                          [h[12], h[13], h[14], h[15]],
                          [-h[1].conj(), h[0].conj(), -h[3].conj(), h[2].conj()],
                          [-h[5].conj(), h[4].conj(), -h[7].conj(), h[6].conj()],
                          [-h[9].conj(), h[8].conj(), -h[11].conj(), h[10].conj()],
                          [-h[13].conj(), h[12].conj(), -h[15].conj(), h[14].conj()],
                          [-h[2].conj(), -h[3].conj(), h[0].conj(), h[1].conj()],
                          [-h[6].conj(), -h[7].conj(), h[4].conj(), h[5].conj()],
                          [-h[10].conj(), -h[11].conj(), h[8].conj(), h[9].conj()],
                          [-h[14].conj(), -h[15].conj(), h[12].conj(), h[13].conj()],
                          [h[3], -h[2], -h[1], h[0]], [h[7], -h[6], -h[5], h[4]], [h[11], -h[10], -h[9], h[8]],
                          [h[15], -h[14], -h[13], h[
                              12]]])  # i kinda manually concatenated the coefficient to assumed the form for an Alamouti Coding. Essentially, H is made to be orthogonal
            H4by4_Uncoded = np.array([[h[0], h[1], h[2], h[3]], [h[4], h[5], h[6], h[7]], [h[8], h[9], h[10], h[11]],
                                      [h[12], h[13], h[14], h[15]]])

            H4by2 = np.array([[h[0], h[4], h[8], h[12]], [h[1], h[5], h[9], h[13]],
                              [h[4].conj(), -h[0].conj(), h[12].conj(), -h[8].conj()],
                              [h[5].conj(), -h[1].conj(), h[13].conj(), -h[9].conj()]])

            n = np.array(n)
            Noise = [n[0], n[1], n[2], n[3], n[4].conj(), n[5].conj(), n[6].conj(), n[7].conj(), n[8].conj(),
                     n[9].conj(), n[10].conj(), n[11].conj(), n[12], n[13], n[14], n[15]]
            Noise4by4_Uncoded = [n[0], n[1], n[2], n[3]]
            Noise4by2 = [n[0], n[1], n[0].conj(), n[1].conj()]

            rx_symbol_Uncoded = np.add((np.dot(H4by4_Uncoded, (np.dot((1 / sqrt(2)), symbol)))),
                                       Noise4by4_Uncoded)  # recieved_symbol, y == rx_symbol
            ...
            no_errors_Uncoded += 1 * (tx_symbol1 != det_symbol1_Uncoded) + 1 * (
                        tx_symbol2 != det_symbol2_Uncoded) + 1 * (tx_symbol3 != det_symbol3_Uncoded) + 1 * (
                                             tx_symbol4 != det_symbol4_Uncoded)
            no_errors += 1 * (tx_symbol1 != det_symbol1) + 1 * (tx_symbol2 != det_symbol2) + 1 * (
                        tx_symbol3 != det_symbol3) + 1 * (tx_symbol4 != det_symbol4)
            no_errors4by2 += 1 * (tx_symbol1 != det_symbol1_4by2) + 1 * (tx_symbol2 != det_symbol2_4by2) + 1 * (
                        tx_symbol3 != det_symbol3_4by2) + 1 * (tx_symbol4 != det_symbol4_4by2)
